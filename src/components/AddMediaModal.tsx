...
export default function AddMediaModal({
  ...
  const searchTMDB = useCallback(async (query: string) => {
    ...
    setLoading(true);
    try {
      const endpoint = defaultType === "film" 
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const insertData = {
        ...
        backdrop_url: selectedItem.backdrop_path ? `https://image.tmdb.org/t/p/original${selectedItem.backdrop_path}` : null,
        year: new Date(selectedItem.release_date || selectedItem.first_air_date).getFullYear() || null,
        rating: selectedItem.vote_average,
        user_rating: listContext === "wantToWatch" ? 0 : userRating,
        ...
        watched_at: listContext === "recentlyWatched" ? new Date().toISOString() : null,
      };
  ...
  const getHeaderInfo = () => {
    const typeLabel = defaultType === "film" ? "un Film" : defaultType === "serie" ? "une Série" : "un Anime";
    switch(listContext) {
      case "topTen": 
        return { title: `Ajouter ${typeLabel} au Top 10`, desc: "Sélectionnez l'élite de votre collection.", icon: <Trophy className="text-amber-500" size={20} /> };
      case "recentlyWatched": 
        return { title: `Ajouter ${typeLabel} aux Vus`, desc: "Gardez une trace de vos visionnages récents.", icon: <History className="text-blue-500" size={20} /> };
      case "wantToWatch": 
        return { title: `Ajouter ${typeLabel} à Voir`, desc: "Planifiez vos futures découvertes.", icon: <Bookmark className="text-emerald-500" size={20} /> };
      default: 
        return { title: "Ajouter un média", desc: "Enrichissez votre Second Brain.", icon: <Plus className="text-blue-500" size={20} /> };
    }
  };
    ...
    <Transition appear show={isOpen} as={Fragment}>
            ...
            <Dialog.Panel className="relative w-full max-w-3xl h-[85vh] md:h-[80vh] flex flex-col overflow-hidden rounded-3xl bg-zinc-900 border border-white/10 shadow-2xl transition-all">
              ...
              <div className="flex items-center justify-between p-5 md:p-6 border-b border-white/5 bg-zinc-900/50 backdrop-blur-md z-10">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <Dialog.Title as="h3" className="text-lg md:text-xl font-bold text-white leading-none">
                      {header.title}
                    </Dialog.Title>
                    <p className="text-xs text-zinc-500 mt-1.5">{header.desc}</p>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 md:p-6 custom-scrollbar">
                ...
                <div className="relative mb-6">
                  <div className="relative group">
                    ...
                    />
                    {loading && <div className="absolute right-4 top-1/2 -translate-y-1/2"><Loader2 className="animate-spin text-blue-500" size={18} /></div>}
                  </div>
                  ...
                  {searchResults.length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-zinc-800 border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-20 max-h-60 overflow-y-auto">
                      {searchResults.map((res) => (
                        <button key={res.id} onClick={() => selectResult(res)} className="w-full p-3 hover:bg-white/5 flex gap-4 text-left transition-colors border-b border-white/5 last:border-0">
                          <div className="relative w-10 h-14 shrink-0 bg-zinc-700 rounded overflow-hidden">
                            {res.poster_path ? <img src={`https://image.tmdb.org/t/p/w92${res.poster_path}`} alt="" className="w-full h-full object-cover" /> : <Film size={14} className="text-zinc-500 m-auto" />}
                          </div>
                          <div className="flex flex-col justify-center">
                            <p className="font-semibold text-white text-sm line-clamp-1">{res.title || res.name}</p>
                            <p className="text-xs text-zinc-500">{res.release_date?.slice(0, 4) || res.first_air_date?.slice(0, 4)}</p>
                          </div>
                        </button>
                      ))}
                    </div>
                ...
                {selectedItem ? (
                    ...
                    <div className="relative overflow-hidden rounded-2xl bg-zinc-800/30 border border-white/5 p-4 md:p-5 flex flex-col sm:flex-row gap-6">
                      ...
                      <div className="relative group shrink-0 mx-auto sm:mx-0">
                        <div className="w-32 md:w-36 aspect-2/3 rounded-xl overflow-hidden shadow-xl border border-white/10">
                          <img src={previewUrl || (selectedItem.poster_path ? `https://image.tmdb.org/t/p/w500${selectedItem.poster_path}` : "/placeholder.png")} alt="" className="w-full h-full object-cover" />
                        </div>
                      ...
                      <div className="flex-1 space-y-3">
                        <h4 className="text-lg font-bold text-white leading-tight">{selectedItem.title || selectedItem.name}</h4>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {mapTmdbGenres(selectedItem.genre_ids).map((genre: string) => (
                            <span key={genre} className="px-2 py-0.5 bg-white/5 border border-white/5 rounded-md text-[10px] text-zinc-300 font-medium flex items-center gap-1">
                        ...
                        <p className="text-xs text-zinc-400 leading-relaxed line-clamp-3 italic">{selectedItem.overview}</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-5">
                        ...
                        {listContext !== "wantToWatch" && (
                          <div>
                            <label className="text-xs font-bold text-zinc-500 uppercase tracking-wider mb-3 flex items-center justify-between">
                              <span className="flex items-center gap-2"><Star size={14} className="text-blue-500" /> Votre note</span>
                              <span className="text-blue-500 font-black text-base">{userRating > 0 ? userRating.toFixed(1) : "--"}</span>
                            </label>
                            <input 
                              type="range" min="0" max="10" step="0.5" value={userRating} 
                        ...
                        {listContext === "topTen" && (
                          <div>
                            <label className="text-xs font-bold text-zinc-500 uppercase tracking-wider mb-3 flex items-center gap-2">
                              <Trophy size={14} className="text-amber-500" /> Classement Top 10
                            </label>
                        ...
                        <div className="space-y-2">
                          <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider">Notes personnelles</label>
                          <textarea 
                      ...
                      <div className="space-y-4">
                        <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider">Statut & Options</label>
                        ...
                        <div className="space-y-3">
                          {listContext === "topTen" && (
                            <div className="p-4 bg-amber-500/5 rounded-2xl border border-amber-500/10 flex items-start gap-3">
                              ...
                              <p className="text-[11px] text-amber-200/60 leading-relaxed">
                                Ce média sera ajouté à votre <strong>Top 10</strong>. Il est automatiquement marqué comme favori et vu.
                              </p>
                            </div>
                          )}
                            ...
                            <>
                              <button 
                                ...
                                className={cn(
                                  ...
                                  favorite ? "bg-rose-500/10 border-rose-500/50 text-rose-400" : "bg-zinc-800/50 border-white/5 text-zinc-400"
                                )}
                              >
                                <div className="flex items-center gap-3">
                                ...
                                <div className={cn("w-5 h-5 rounded-full border-2 flex items-center justify-center", favorite ? "border-rose-500 bg-rose-500" : "border-zinc-600")}>
                                  {favorite && <div className="w-1.5 h-1.5 bg-white rounded-full" />}
                                </div>
                              </button>
                              <div className="p-3 bg-blue-500/5 rounded-xl border border-blue-500/10 flex items-center gap-2">
                                <Eye size={14} className="text-blue-400" />
                                <p className="text-[10px] text-blue-400/70">Sera marqué comme vu aujourd hui.</p>
                              </div>
                            </>
                          )}
                          ...
                          {listContext === "wantToWatch" && (
                            <div className="p-4 bg-emerald-500/5 rounded-2xl border border-emerald-500/10 flex items-start gap-3">
                              <Bookmark className="text-emerald-500 shrink-0" size={18} />
                              <p className="text-[11px] text-emerald-200/60 leading-relaxed">
                                Ajouté à votre liste <strong>À regarder</strong>. La note et les favoris sont désactivés car vous ne l avez pas encore vu.
                              </p>
                            </div>
                          )}
                        </div>
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-center py-16 space-y-4">
                    ...
                    <div>
                      <h4 className="text-base font-medium text-white">Trouver {defaultType === "film" ? "un film" : defaultType === "serie" ? "une série" : "un anime"}</h4>
                      <p className="text-xs text-zinc-500 mt-1">Recherchez pour importer les données TMDB.</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="p-5 md:p-6 border-t border-white/5 bg-zinc-900/80 backdrop-blur-md flex justify-end gap-3">
                <button onClick={onClose} className="px-5 py-2 rounded-xl text-sm font-medium text-zinc-400 hover:bg-white/5 transition-all">Annuler</button>
                <button 
                  onClick={handleSubmit} 
                  disabled={loading || !selectedItem || (listContext === "topTen" && priority === null)} 
                  className={cn(
                    "px-7 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2",
                    loading || !selectedItem || (listContext === "topTen" && priority === null) ? "bg-zinc-800 text-zinc-600" : "bg-blue-600 text-white hover:bg-blue-500 shadow-lg shadow-blue-600/20"
                  )}
                >
      ...
      <style jsx global>{`
        ...
        input[type='range']::-webkit-slider-thumb {
          appearance: none; width: 16px; height: 16px; background: #3b82f6; border-radius: 50%; cursor: pointer; border: 2px solid #18181b;
        }
      `}</style>
    </Transition>
  );
